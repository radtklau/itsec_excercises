...
class HA2_2:
    ...
    def decrypt_block(block_l,block_r,server):
        for byte_no in range(16):
            ...
            p2[-(byte_no+1)] = c1[-(byte_no+1)] ^ i2[-(byte_no+1)]
            print(bytes(p2))
    ...
    def multip(self):
        jobs = []
        for i in range(3):
            block = mp.Process(target=self.decrypt_block, args = (blocks[-(i+2)], blocks[-(i+1)], server))
            jobs.append(block)
            block.start()
...
if __name__ == "__main__":
    ...
    server = SSHClient()
    server.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    server.connect('gruenau5.informatik.hu-berlin.de',username='radtklau',password='')
